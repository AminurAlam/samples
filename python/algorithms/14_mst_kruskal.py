class Graph:
    def __init__(self, G: list[list[int]]):
        self.V: int = max([x[0] for x in G] + [x[1] for x in G])
        self.G: list[list[int]] = sorted(G, key=lambda item: item[2])
        self.parent: list[int] = list(range(len(G)))
        self.rank: list[int] = [1] * len(G)
    ...
    def mst(self):
        ...
        for node in self.G:
            x, y = self.find(self.parent, node[0]), self.find(self.parent, node[1])
            if x == y:
                continue
            result.append(node)
